  ...
  const AGENTS = [
    {
      ...
      desc: "Busca 50+ leads/día en Apollo.io. 5 verticales: restaurantes, hoteles, retail, hospitales, corporativos. Guarda con todos los campos para ZEUS.",
      stats: [`${totalLeads} leads en pipeline`, "5 verticales activas"],
      status: "ACTIVO",
      outputs: ["ZEUS"],
    },
    {
      id: "ZEUS", name: "ZEUS", icon: "🧠", color: "#6366f1",
      role: "Scoring + Routing", schedule: "Cada 15 min",
      desc: "Califica 0-100. HOT/WARM/COLD. Si tiene teléfono → MERCURY (WhatsApp). Si no → HERMES (email). VIP → alerta directa a Iván.",
      stats: [`${hotLeads} HOT activos`, "Phone → WA | No phone → Email"],
      status: "ACTIVO",
      inputs: ["APOLO"],
    ...
    },
    {
      ...
      desc: "Template aprobado Meta. Cadencia A (3 días) y B (90 días). Si no hay teléfono, cae automáticamente a HERMES. Fallback inteligente.",
      stats: [`${waSent} WA enviados`, "Template botmate_intro APROBADO ✅"],
      status: "ACTIVO",
      inputs: ["ZEUS"],
      outputs: ["SIGMA","RESCATE"],
    },
    {
      id: "HERMES", name: "HERMES v5", icon: "📧", color: "#2563eb",
      role: "Email Outreach + Follow-up", schedule: "Webhook (ZEUS)",
      desc: "Emails HTML personalizados con Claude. Sin markdown. Contexto estacional México. ROI calculado. BCC a Iván. HERMES-REPLY automático cuando responden.",
      stats: [`${emailSent} emails enviados`, "HERMES-REPLY activo 🆕"],
      status: "ACTIVO",
      inputs: ["ZEUS"],
      outputs: ["ATHENA","SIGMA"],
    },
    {
      id: "ATHENA", name: "ATHENA", icon: "👁", color: "#0891b2",
      role: "Email Reply Detector", schedule: "Cada 5 min (Gmail)",
      desc: "Google Apps Script monitorea inbox. Detecta respuestas positivas/negativas. Clasifica intención. Dispara HERMES-REPLY. Alerta Telegram <5min.",
      stats: ["Gmail monitoreado 24/7", "Positivo → follow-up automático"],
      status: "ACTIVO",
      inputs: ["HERMES"],
      outputs: ["HERMES","SIGMA"],
    },
    {
      id: "SIGMA", name: "SIGMA", icon: "📋", color: "#a855f7",
      role: "Propuestas Automáticas", schedule: "Webhook",
      desc: "Genera y envía propuesta comercial HTML completa. Tabla de precios, ROI calculado, casos de éxito por sector. Disparado cuando lead muestra interés real.",
      stats: [`${proposals} propuestas enviadas`, "ROI + pricing automático"],
      status: "ACTIVO",
      inputs: ["ATHENA","MERCURY"],
      outputs: ["ARES"],
    },
    {
      id: "RESCATE", name: "RESCATE", icon: "🚨", color: "#f97316",
      role: "Último intento", schedule: "Webhook (Cadencia)",
      desc: "Cuando el lead no responde a toda la cadencia. Claude genera mensaje con ángulo diferente. Envía WA si hay teléfono, email si no. Último intento antes de archivar.",
      stats: ["Ángulo diferente con IA", "WA o email según canal"],
      status: "ACTIVO",
      inputs: ["MERCURY"],
      outputs: [],
    },
    {
      id: "ARES", name: "ARES", icon: "🤝", color: "#16a34a",
      role: "Post-Venta + Renovaciones", schedule: "9AM diario",
      desc: "Check-in días 2, 7, 14, 30 post-instalación. Alerta renovación 30 días antes. Referidos día 30. Activa cuando llega el primer cliente.",
      stats: [`${clientes.length} clientes activos`, "Renovaciones monitoreadas"],
      status: clientes.length > 0 ? "ACTIVO" : "EN ESPERA",
      inputs: ["SIGMA"],
      outputs: [],
    },
    {
      id: "MORNING", name: "ZEUS REPORT", icon: "☀️", color: "#06b6d4",
      role: "Briefing Matutino", schedule: "7AM Lun-Vie",
      desc: "Telegram 7AM: pipeline completo, quién respondió, seguimientos del día, leads HOT sin respuesta, sistema autónomo activo.",
